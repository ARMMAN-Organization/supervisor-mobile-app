...
/** Monitoring Summary detail screen: location filter + one Village/Mother/Child table per Sakhi. */
@Composable
fun MonitoringSummaryScreen(
  onBack: () -> Unit,
  modifier: Modifier = Modifier,
  viewModel: MonitoringSummaryViewModel = hiltViewModel(),
) {
  val uiState by viewModel.uiState.collectAsStateWithLifecycle()

  Scaffold(
    modifier = modifier,
    topBar = { BrandTopAppBar(title = stringResource(R.string.monitoring_summary_title), onBack = onBack) },
  ) { innerPadding ->
    BoxWithConstraints(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
      val isTablet = maxWidth >= Dimens.TabletMinWidthDp.dp

      when (val state = uiState) {
        is MonitoringSummaryUiState.Loading -> LoadingContent()
        is MonitoringSummaryUiState.Error -> ErrorContent(state = state, onRetry = viewModel::onRetry)
        is MonitoringSummaryUiState.Success -> SuccessContent(
          state = state,
          isTablet = isTablet,
...
@Composable
private fun ErrorContent(state: MonitoringSummaryUiState.Error, onRetry: () -> Unit) {
  Column(
...
private fun SuccessContent(
  state: MonitoringSummaryUiState.Success,
  isTablet: Boolean,
  ...
  Column(
    ...
    if (state.sakhis.isEmpty()) {
      Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Text(text = stringResource(R.string.monitoring_summary_empty), style = MaterialTheme.typography.bodyLarge)
      }
    } else {
