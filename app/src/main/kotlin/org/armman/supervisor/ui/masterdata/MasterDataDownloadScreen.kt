...
/**
 * Download Master Data screen: a live-updating list of master-data tables (Pending →
 * Downloading → Completed/Empty/Not available), auto-scrolling to the active row, with
 * quit-confirmation on back and a completion dialog once the whole chain finishes.
 */
@Composable
fun MasterDataDownloadScreen(
  onBack: () -> Unit,
  modifier: Modifier = Modifier,
  viewModel: MasterDataDownloadViewModel = hiltViewModel(),
) {
  // Exhaustive per this app's sealed-UiState convention — MasterDataDownloadUiState has only one
  // variant today, but dispatching through `when` (not a direct cast) means the compiler forces a
  // new branch here the moment a second variant is ever added, instead of that being silently
  // ignored the way a bare `is Content ->` cast would be.
  when (val state = viewModel.uiState.collectAsStateWithLifecycle().value) {
    is MasterDataDownloadUiState.Content ->
...
private fun ContentScreen(
  state: MasterDataDownloadUiState.Content,
  onBack: () -> Unit,
  modifier: Modifier,
  viewModel: MasterDataDownloadViewModel,
) {
  val listState = rememberLazyListState()

  // Alignment intent: the active row is scrolled to the top-visible position, matching the
  // reference app's "auto-scrolls to the row currently in progress" behavior.
  ...
  Scaffold(
    modifier = modifier,
    topBar = {
      BrandTopAppBar(title = stringResource(R.string.master_data_download_title), onBack = ::handleBack)
    },
  ) { innerPadding ->
    BoxWithConstraints(
...
@Composable
private fun MasterDataRow(row: DownloadRow, isActive: Boolean) {
  ...
  val rowContentDescription = stringResource(R.string.master_data_row_content_description, label, statusText)
  ...
  Surface(
    ...
    modifier = Modifier
      ...
      // "Block: Completed") instead of two separate reads, and re-fires whenever the status text
      // changes so a screen reader hears each status transition, not just the initial render.
      .clearAndSetSemantics { contentDescription = rowContentDescription }
      .testTag("master_data_row_${row.entity.name}"),
  ) {
    ...
    Column(modifier = Modifier.fillMaxWidth().clip(cardShape)) {
      ...
      if (row.status == DownloadRowStatus.DOWNLOADING) {
...
@Composable
private fun statusLabelAndColor(status: DownloadRowStatus): Pair<String, androidx.compose.ui.graphics.Color> =
  when (status) {
...
private fun QuitConfirmationDialog(onConfirm: () -> Unit, onDismiss: () -> Unit) {
  AlertDialog(
    ...
    title = { Text(stringResource(R.string.master_data_quit_title)) },
    text = { Text(stringResource(R.string.master_data_quit_message)) },
    confirmButton = {
      TextButton(onClick = onConfirm) {
        Text(stringResource(R.string.master_data_quit_confirm), color = RiskHigh)
      }
    },
    dismissButton = {
      TextButton(onClick = onDismiss) { Text(stringResource(R.string.master_data_quit_cancel)) }
    },
  )
}
...
private fun NetworkErrorDialog(onRetry: () -> Unit, onStop: () -> Unit) {
  AlertDialog(
    ...
    title = { Text(stringResource(R.string.master_data_network_error_title)) },
    text = { Text(stringResource(R.string.master_data_network_error_message)) },
    confirmButton = { TextButton(onClick = onRetry) { Text(stringResource(R.string.master_data_retry)) } },
    dismissButton = { TextButton(onClick = onStop) { Text(stringResource(R.string.master_data_stop)) } },
  )
}
...
private fun CompletionDialog(onConfirm: () -> Unit) {
  AlertDialog(
    ...
    title = { Text(stringResource(R.string.master_data_completion_title)) },
    text = { Text(stringResource(R.string.master_data_completion_message), color = StatusSuccess) },
    confirmButton = { TextButton(onClick = onConfirm) { Text(stringResource(R.string.master_data_ok)) } },
  )
}
